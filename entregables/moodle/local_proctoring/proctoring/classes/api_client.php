...
class api_client {
    ...
    public function get_session_status(string $sessionid): string {
        $decoded = $this->request(
            '/v1/internal/sessions/' . rawurlencode($sessionid) . '/status',
            []
        );

        if (empty($decoded['session']['status'])) {
            throw new \moodle_exception('apiunavailable', 'local_proctoring');
        }

        return $decoded['session']['status'];
    }
