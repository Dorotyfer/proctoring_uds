"""Node-compatible AES-256-GCM evidence encryption."""
...
class EncryptedEvidence:
  ...
  def wire_object(self) -> dict[str, bytes]:
    """Match Node's separate ciphertext, IV, and authentication-tag fields."""

    return {"ciphertext": self.ciphertext, "iv": self.iv, "tag": self.tag}


class EvidenceEncryptionService:
  """AES-256-GCM with Node's 12-byte nonce and detached 16-byte tag format."""
  ...
  def __init__(self, key: bytes, *, key_version: str | None = None) -> None:
    ...
    # Historical Node evidence has no key-version column. Keep this constructor
