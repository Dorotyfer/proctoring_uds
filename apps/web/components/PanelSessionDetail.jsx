  ...
  return (
    <section className="panel-content panel-detail" aria-labelledby="session-title">
      ...
      <dl className="detail-summary">
        ...
        <div><dt>Estado</dt><dd>{sessionStatusLabel(session.status)}</dd></div>
        <div><dt>Modalidad</dt><dd>{deviceModeLabel(session.deviceMode)}</dd></div>
        ...
        <table className="alert-table">
          ...
          <tbody>
            {session.alerts.map((alert) => <tr key={alert.id}>
              ...
              <td data-label="Severidad"><span className={`badge ${alert.severity === 'high' ? 'warning' : ''}`}>{severityLabel(alert.severity)}</span></td>
      ...
      {incidentEvidence.length === 0 ? <p>No hay imágenes asociadas a incidencias.</p> : incidentEvidence.map((item) => (
        <button className="text-button evidence-link" type="button" key={item.id} onClick={() => onEvidence(item.id)}>Ver incidencia · {formatDateTime(item.created_at)}</button>
      ))}
      <section className="event-timeline" aria-label="Cronología de eventos">
        <h3>Cronología</h3>
        {session.events.map((event) => <article className="timeline-item" key={event.id}><strong>{alertTypeLabel(event.type)}</strong>{event.type === 'attention_signal' ? <p>{attentionSignalLabel(event.metadata)}</p> : null}<p>{formatDateTime(event.occurred_at)}</p></article>)}
      </section>
    </section>
  );
}
...
function attentionSignalLabel(metadata = {}) {
  const expression = expressionLabel(metadata.expression);
  ...
  return (
    <section className="behavior-analysis" aria-labelledby="behavior-analysis-title">
      ...
      <section aria-label="Señales detectadas" className="risk-signals">
        ...
        {risk.reasons?.length || risk.factors?.length ? (risk.reasons ?? risk.factors).map((reason) => {
          const code = reason.code ?? reason.type;
          const points = reason.points ?? reason.contribution;
          return <div className="risk-signal" key={code}>
            <div><strong>{reason.label || alertTypeLabel(code)}</strong><span>{reason.count} evento(s) · {points} puntos</span></div>
            <div className="risk-bar" aria-label={`${reason.label || alertTypeLabel(code)}: ${points} puntos`}><span style={{ width: `${Math.min(points / 35 * 100, 100)}%` }} /></div>
          </div>;
        }) : <p>Sin señales relevantes detectadas.</p>}
      </section>
