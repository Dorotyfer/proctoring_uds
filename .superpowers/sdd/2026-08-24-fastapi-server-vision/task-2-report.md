...
## Final validation-details round

Completed `CreateSessionInput` Zod issue translation for missing and wrong types, min/max strings (including the 100-character nullable document limit), browser/SEB enum values and type errors, RFC3339 datetime errors and type errors, nullable document acceptance, the expiration custom issue, and mixed multi-field payloads. All reported paths use camelCase wire aliases. Raw malformed/non-object JSON behavior remains the existing no-details response.

RED command:

```powershell
& .\.venv\Scripts\python.exe -m pytest apps\api\tests\test_sessions_events.py::test_session_validation_details_match_legacy_zod_issue_families -q
```

RED output: `7 failed, 1 passed`; the incomplete helper emitted generic `custom` Pydantic messages for wrong types, length limits, datetime, and the model-level expiration issue.

Additional RED command after extending enum/datetime wrong-type coverage:

```powershell
& .\.venv\Scripts\python.exe -m pytest apps\api\tests\test_sessions_events.py::test_session_validation_details_match_legacy_zod_issue_families -q
```

RED output: `2 failed, 9 passed`; numeric enum and datetime inputs still needed Node-family `invalid_type` mapping.
...
GREEN output: `39 passed in 2.18s`.
...
GREEN output: `98 passed in 2.42s`; `git diff --check` completed without errors.

Final validation-details implementation commit: `79eab78a27f079725c64979df563ccc8eef3e819` (`fix: complete legacy session validation details`).
